...
// 🚨 NEW: ENFORCE CHANNEL MEMBERSHIP (The Leaver Punisher)
    ...
    if (chatId === config.TARGET_GROUP_ID) {
        ...
        if (isJoin && userId !== state.botId) {
            ...
            if (['talk', 'media', 'channel'].includes(state.botConfig.verifyMode)) {
                ...
                try {
                    const me = await bot.getMe();
                    let kb = [[{ text: "✅ Tap to Verify", url: `https://t.me/${me.username}?start=verify_${chatId}` }]];
                    
                    const sentMsg = await bot.sendMessage(chatId, welcomeMsg, { parse_mode: 'HTML', reply_markup: { inline_keyboard: kb } });
    ...
    // ... rest of your code
        ...
        // 1. Ensure the structures exist in activityStats (Daily/All-Time)
        if (!state.activityStats.GLOBAL_STATS) {
            state.activityStats.GLOBAL_STATS = { daily: 0, allTime: 0 };
        }
        // 2. Ensure structures exist in monthlyActivity
        if (!state.monthlyActivity.GLOBAL_STATS) {
            state.monthlyActivity.GLOBAL_STATS = { monthly: 0 };
        }

        // 3. Update User Counts (Daily)
        if (!state.activityStats[uid]) {
            state.activityStats[uid] = { name: msg.from.first_name || "User", count: 0 };
        }
        state.activityStats[uid].count += 1;

        // 4. Update User Counts (Monthly)
        if (!state.monthlyActivity[uid]) {
            state.monthlyActivity[uid] = { name: msg.from.first_name || "User", count: 0 };
        }
        state.monthlyActivity[uid].count += 1;
